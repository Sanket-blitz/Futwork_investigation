...
def process_data(file_path):
    ...
    def extract_recording_link(response):
        try:
            ...
            return proof_link if proof_link and proof_link.startswith("https://recordings.futwork.com") else None
    ...
    filtered_df.loc[:, 'Response Time'] = pd.to_datetime(filtered_df['Response Time'])
    ...
    hourly_samples = filtered_df.groupby(filtered_df['Response Time'].dt.hour, group_keys=False).apply(sample_hour)
    ...
    hourly_samples.to_excel(output_file_path, index=False)
